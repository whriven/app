        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(skuSaleAttr,index) in spuSaleAttrList" :key="skuSaleAttr.id">
                <dt class="title">选择{{skuSaleAttr.saleAttrName}}</dt>
                <dd 
                changepirce="0" 
                @click="changeActive(skuSaleAttr.spuSaleAttrValueList,attrValue)" 
                :class="{active:attrValue.isChecked==1}" 
                v-for="(attrValue,index) in skuSaleAttr.spuSaleAttrValueList" 
                :key="attrValue.id">
            ...
            <div class="cartWrap">
              ...
              <div class="add" @click="addShopCart(skuInfo.id,skuNum)">
...
export default {
  ...
  mounted() {
    this.$store.dispatch("getDoodsDetail", this.$route.params.skuId);
  },
  computed: {
    ...mapState({
  ...
  methods: {
    ...
    async addShopCart(skuId,skuNum) {
      try {
        ...
        // try结构体里的语句是成功时执行的语句
        // 添加购物车成功之后把相应的产品信息存储在sessionStorage中
        sessionStorage.setItem('SKUINFO',JSON.stringify(this.skuInfo))
        // 添加购物车返回的是成功之后要进行路由跳转
        this.$router.push({
          name:'addcartsuccess',
